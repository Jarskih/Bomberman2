...
TextHandler::TextHandler(SDL_Renderer* p_renderer)
{
...
sp<Sprite> TextHandler::CreateText(std::string p_text, SDL_Color p_textColor, TTF_Font* p_font, int p_x, int p_y, int p_w, int p_h)
{
	...
	else
	{
		...
		const auto sprite = makesp<Sprite>(*textTexture, p_x, p_y, p_w, p_h);
		return sprite;
	}
	return nullptr;
}
